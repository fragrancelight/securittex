            ...
            if ($data['wallet']->coin_type == 'USDT' && $data['wallet']->network == COIN_PAYMENT) {
                ...
                if (isset($exists) && (!empty($exists->address))) {
                    ...
                    if (!empty($address['address'])) {
                        ...
                        $history->AddWalletAddressHistory($data['wallet']->id, $address['address'], $data['wallet']->coin_type,$address['wallet_key'],$address['public_key']);
                    }
                }
            }
...
//        dd($transactions);
    ...
    {
        try {
            $from_coin_info = Coin::where('coin_type','=',$from_coin_type)->first();
    ...
    public function getMyWalletListWithOnorderWithTotal($user_id,$paginate = null)
    {
        $dashboardRepo = new DashboardRepository();
        $total = 0;
        $wallets = Wallet::join('coins','coins.id', '=', 'wallets.coin_id')
            ->where(['wallets.user_id'=> $user_id, 'wallets.type'=> PERSONAL_WALLET, 'coins.status' => STATUS_ACTIVE])
            ->orderBy('wallets.id', 'ASC')
            ->select('wallets.*', 'coins.coin_icon', 'coins.is_withdrawal', 'coins.is_deposit','coins.trade_status')
            ->get();
        if (isset($wallets[0])) {
    ...
    // get my wallet list with on order with total
    public function getMyWalletListWithOnorderWithTotalWithoutUSD($user_id,$paginate = null)
    {
        $dashboardRepo = new DashboardRepository();
        $total = 0;
        $wallets = Wallet::join('coins','coins.id', '=', 'wallets.coin_id')
