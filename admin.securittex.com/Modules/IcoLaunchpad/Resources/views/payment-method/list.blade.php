    ...
    <div class="custom-breadcrumb">
        <div class="row">
            ...
            <div class="col-sm-7 text-right">
                <a class="add-btn theme-btn" href="{{route('IcoPaymentMethodAdd')}}">{{__('Add New')}}</a>
            </div>
        </div>
    </div>
    <!-- /breadcrumb -->
                            ...
                            <table id="table" class="table table-borderless custom-table display text-center" width="100%">
                                ...
                                @if(isset($items))
                                    @foreach($items as $value)
                                        <tr>
                                            <td> {{ $value->title}} </td>
                                            <td> {{htmlPaymentMethod($value->payment_method)}} </td>
                                            <td>
                                                <div>
                                                    <label class="switch">
                                            ...
                                            <td>
                                                <ul class=" d-flex activity-menu">
                                                    <li>
                                                        <a title="{{__('Edit')}}" href="{{ route("paymentMethodAddEdit",["id" => $value->id]) }}">
                                                            <i class="fa fa-pencil"></i>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a title="{{__('Delete')}}" href="{{ route("paymentMethodAddDelete",["id" => $value->id]) }}">
...
@section('script')
    <script>
        ...
        function statusChange(currency_payment_method_id) {
            $.ajax({
                type: "POST",
                url: "{{ route('paymentMethodAddStatus') }}",
                data: {
                    '_token': "{{ csrf_token() }}",
