...
@section('content')
    ...
    @php
        $title = null;
        $body = null;
        if (isset($news_post_details)) {
            $title = $news_post_details->title;
            $body = $news_post_details->body;
        }
        if (isset($news_post_translation)) {
            $title = $news_post_translation->title;
            $body = $news_post_translation->body;
        }
    @endphp
    <div class="user-management">
        <form action="{{ route('newsPostTranslateUpdateText') }}" method="post">
            @csrf
            @if(isset($news_post_details))
                <input type="hidden" value="{{ $news_post_details->id }}" name="news_post_id" />
            @endif
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        <div class="form-group">
                            ...
                            <input name="title" class="form-control" value="{{ $title }}" type="text" placeholder="{{ __("News Post Title") }}" />
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
